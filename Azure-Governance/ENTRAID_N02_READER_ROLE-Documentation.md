# Entra ID Security Group - N02 Reader Role

## 🎯 Group Configuration

**Group Name:** `ENTRAID_N02_READER_ROLE`  
**Purpose:** Security group for N02 subscription Reader access  
**Owner:** Amir.Shahzad@condoauthorityontario.ca  
**Member:** greg.beaver@condoauthorityontario.ca  
**Role Assignment:** Ready for Reader role assignment to N02 subscription
...
### Target Subscription
- **Subscription:** N02 (to be identified and assigned)
...
- **greg.beaver@condoauthorityontario.ca**
  - Will gain Reader access to N02 subscription when assigned
  - Can view all resources and configurations
  - Cannot modify any resources

## 🔧 Role Assignment Instructions

To complete the setup, assign this group to the N02 subscription:

### Find N02 Subscription
```bash
# Search for N02 subscription
az account list --query "[?contains(toLower(name), 'n02')].{Name:name, Id:id}" --output table
```

### Assign Reader Role
```bash
# Get the group object ID
GROUP_ID=$(az ad group show --group "ENTRAID_N02_READER_ROLE" --query "id" --output tsv)

# Assign Reader role to N02 subscription
az role assignment create \
    --assignee $GROUP_ID \
    --role "Reader" \
    --scope "/subscriptions/YOUR_N02_SUBSCRIPTION_ID"
```
...
3. **Switch to N02 subscription** if multiple subscriptions are available
4. **Browse resources** with read-only access
5. **View monitoring** and diagnostic information

### For Amir Shahzad (Owner)
1. **Complete role assignment** to N02 subscription
...
# Add to group
az ad group member add --group "ENTRAID_N02_READER_ROLE" --member-id $USER_ID
```
...
# Remove from group
az ad group member remove --group "ENTRAID_N02_READER_ROLE" --member-id $USER_ID
```

### Check Group Status
```bash
# List all members
az ad group member list --group "ENTRAID_N02_READER_ROLE" --query "[].{DisplayName:displayName, UserPrincipalName:userPrincipalName}" --output table

# List all owners
az ad group owner list --group "ENTRAID_N02_READER_ROLE" --query "[].{DisplayName:displayName, UserPrincipalName:userPrincipalName}" --output table
...
- **Scope:** Limited to N02 subscription only
- **Review:** Regular access reviews required

## 🎯 Extended Security Group Portfolio

This group extends the comprehensive subscription-level Reader access strategy:

### Production/Business Subscriptions
- **ENTRAID_N01_READER_ROLE** → N01 subscription
- **ENTRAID_N02_READER_ROLE** → N02 subscription ← **NEW**
...
## 🔍 N01 vs N02 Distinction

Since both N01 and N02 subscriptions exist, consider:
- **Purpose Differentiation:** What distinguishes N01 from N02?
- **Environment Separation:** Different business units or purposes?
- **Access Patterns:** Same users need access to both or different teams?
- **Governance:** Consistent or different compliance requirements?

## 📞 Support

For issues with this security group:
1. **Group Management:** Contact Amir.Shahzad@condoauthorityontario.ca
2. **Role Assignment:** Complete assignment to N02 subscription when ready
...
**Status:** Group created, pending N02 subscription role assignment